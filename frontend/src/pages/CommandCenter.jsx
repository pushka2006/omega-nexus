...
const QUICK_LINKS = [
  ...
  { to: "/agents",    icon: Network,       label: "Agents Hub",      color: "#6E56FF", desc: "35+ specialized AIs" },
  { to: "/memory",    icon: Brain,         label: "Memory Vault",    color: "#FF2E88", desc: "7 global memory types" },
  { to: "/knowledge", icon: Library,       label: "Knowledge Base",  color: "#00FF88", desc: "RAG & license aware" },
  { to: "/code",      icon: Code2,         label: "Software Factory", color: "#FFC857", desc: "SaaS, Mobile, IoT, Games" },
  { to: "/terminal",  icon: Terminal,      label: "Terminal",        color: "#00F5FF", desc: "NEXUS shell" },
  { to: "/monitor",   icon: Activity,      label: "Compute & GPUs",  color: "#00FF88", desc: "Distributed GPU Clusters" },
  { to: "/camera",    icon: Camera,        label: "Robotics & Smart City", color: "#FF2E88", desc: "Drones, ROS, IoT, Vehicles" },
  { to: "/tasks",     icon: ListTodo,      label: "Canary Deployments", color: "#6E56FF", desc: "K8s, Canary, Blue-Green" },
];
...
export default function CommandCenter() {
  ...
  const startBtScan = async () => {
    ...
    if (navigator.bluetooth) {
      ...
      try {
        ...
        if (device) {
          const newDev = { name: device.name || "Bluetooth Device", connected: true, id: device.id };
          setBtDevices(prev => {
            const exists = prev.some(d => d.name === newDev.name);
            if (!exists) return [newDev, ...prev];
            return prev.map(d => d.name === newDev.name ? { ...d, connected: true } : d);
          });
          toast.success(`Paired with ${device.name || "Device"}`);
        }
      } catch (err) {
        if (err.name !== "NotFoundError") {
          toast.error("Bluetooth scan error: " + err.message);
        }
      } finally {
        setBtScanning(false);
      }
    } else {
      toast.info("Web Bluetooth API not supported in this browser. Showing host devices.");
      fetchRealBtDevices();
    }
  };

  const toggleDeviceConnect = (deviceName) => {
    setBtDevices(prev => prev.map(d => {
      if (d.name ===deviceName) {
        const nextState = !d.connected;
        toast.info(`${deviceName} is now ${nextState ? "connected" : "disconnected"}`);
  ...
  // Screencast — real WebRTC Display Media API
  const toggleScreencast = async () => {
    if (screencastOn) {
      ...
      toast.success("Screencast session stopped");
    } else {
      try {
        toast.info("Requesting screen capture permission...");
        ...
        setCastInfo({
          ...
          label: videoTrack.label || "Display Screen"
        });
        ...
        videoTrack.onended = () => {
          ...
          setScreencastOn(false);
          setCastInfo({ width: 0, height: 0, fps: 0, label: "" });
          toast.info("Screencast ended by user");
        };

        toast.success(`Screencast active — ${settings.width}x${settings.height} @ ${settings.frameRate || 30}fps`);
      } catch (err) {
        if (err.name !== "NotAllowedError") {
          toast.error("Failed to start screencast: " + err.message);
        }
      }
    }
  };
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [s, m, cn, dp, pr, xl, rt] = await Promise.all([
        http.get("/stats"),
        http.get("/metrics"),
        http.get("/compute/nodes"),
        http.get("/deployments/pipeline"),
        http.get("/projects"),
        http.get("/explainability/logs"),
        http.get("/robotics/telemetry")
      ]);
      setStats(s.data);
      setMetrics(m.data);
      if (cn.data) setComputeNodes(cn.data);
      if (dp.data) setDeployments(dp.data);
      if (pr.data) setProjects(pr.data);
      if (xl.data) setXaiLogs(xl.data);
      if (rt.data) setRobotics(rt.data);
    } catch { /* offline */ }
  }, []);
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 300px", gap: 20 }}>
        ...
        <div style={{ gridColumn: "span 2", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Real Working Projects Panel */}
          {projects.length > 0 && (
            <div className="nx-glass" style={{ borderRadius: 12, padding: "16px 20px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <Code2 style={{ width: 14, height: 14, color: "#00F5FF" }} />
                  <span className="hud-label">REAL WORKING PROJECTS ({projects.length})</span>
                </div>
                <span style={{ fontSize: 9.5, color: "#34d399", fontFamily: "monospace", fontWeight: 700 }}>● ALL DEPLOYMENTS LIVE</span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {projects.map(p => (
                  <div key={p.id} style={{ background: "rgba(15,23,42,0.6)", border: "1px solid rgba(0,245,255,0.15)", borderRadius: 8, padding: "10px 12px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                    <div>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontSize: 11.5, fontWeight: 750, color: "#e2e8f0", fontFamily: "'Space Grotesk',sans-serif" }}>{p.name}</span>
                        <span style={{ fontSize: 9, color: "#00FF88", background: "rgba(0,255,136,0.1)", border: "1px solid rgba(0,255,136,0.25)", padding: "1px 5px", borderRadius: 4, fontFamily: "monospace" }}>{p.monetization_mrr}</span>
                      </div>
                      <div style={{ fontSize: 9.5, color: "rgba(148,163,184,0.6)", fontFamily: "monospace", marginBottom: 6 }}>{p.category} · {p.health}</div>
                    </div>
                    <a href={p.deployment_url} target="_blank" rel="noreferrer" style={{ textDecoration: "none" }}>
                      <button style={{ width: "100%", padding: "5px", borderRadius: 5, background: "rgba(0,245,255,0.1)", border: "1px solid rgba(0,245,255,0.3)", color: "#00F5FF", fontSize: 9.5, fontFamily: "monospace", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
                        <ExternalLink style={{ width: 10, height: 10 }} /> LAUNCH LIVE APP ↗
                      </button>
                    </a>
                  </div>
                ))}
          ...
          {/* Active Deployment Pipelines Panel */}
          {deployments.length > 0 && (
            <div className="nx-glass" style={{ borderRadius: 12, padding: "16px 20px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <Tv style={{ width: 14, height: 14, color: "#6E56FF" }} />
                  <span className="hud-label">ACTIVE DEPLOYMENT PIPELINES ({deployments.length})</span>
                </div>
                <span style={{ fontSize: 9.5, color: "#6E56FF", fontFamily: "monospace", fontWeight: 700 }}>● CANARY & BLUE-GREEN</span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {deployments.map(d => (
                  <div key={d.id} style={{ background: "rgba(15,23,42,0.6)", border: "1px solid rgba(110,86,255,0.2)", borderRadius: 8, padding: "10px 12px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#e2e8f0", fontFamily: "monospace" }}>{d.app_name}</div>
                      <div style={{ fontSize: 9, color: "rgba(148,163,184,0.6)", fontFamily: "monospace", marginTop: 2 }}>
                        Strategy: <span style={{ color: "#a5b4fc" }}>{d.strategy.toUpperCase()}</span> · Cluster: {d.kubernetes_cluster} · Replicas: {d.active_replicas}
                      </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 11, fontWeight: 800, color: "#34d399", fontFamily: "monospace" }}>{d.health_score}% SLA</div>
                        <div style={{ fontSize: 8.5, color: "rgba(148,163,184,0.5)", fontFamily: "monospace" }}>HEALTHY</div>
                      </div>
                      <a href={d.url} target="_blank" rel="noreferrer" style={{ textDecoration: "none" }}>
                        <button style={{ padding: "5px 9px", borderRadius: 5, background: "rgba(110,86,255,0.15)", border: "1px solid rgba(110,86,255,0.3)", color: "#a5b4fc", fontSize: 9, fontFamily: "monospace", fontWeight: 700, cursor: "pointer" }}>
                          OPEN ↗
                        </button>
                      </a>
                    </div>
