...
export default function ProjectsHub() {
  ...
  const [codeModalViewMode, setCodeModalViewMode] = useState("preview");
  ...
  const openCodeInspector = async (slugName, initialViewMode = "preview") => {
    setCodeModalSlug(slugName);
    setCodeModalOpen(true);
    setCodeModalViewMode(initialViewMode);
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      {/* ── DEVELOPER FULL-STACK CODE & LIVE PREVIEW MODAL ── */}
      {codeModalOpen && (() => {
        const cleanSlug = (codeModalSlug || "").replace(/^proj-/, "");
        const liveAppUrl = `/apps/${cleanSlug}/index.html`;

        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(2,6,23,0.92)", backdropFilter: "blur(14px)", zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
            onClick={() => setCodeModalOpen(false)}
          >
            <div onClick={e => e.stopPropagation()} className="nx-fadein"
              style={{ width: "100%", maxWidth: 1180, height: "90vh", borderRadius: 20, background: "#060d22", border: "1px solid #00F5FF", boxShadow: "0 0 70px rgba(0,245,255,0.25)", padding: 18, display: "flex", flexDirection: "column", gap: 12, overflow: "hidden" }}
            >
              {/* Top Header */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid rgba(0,245,255,0.2)", paddingBottom: 10, flexWrap: "wrap", gap: 10 }}>
                <div>
                  <div style={{ fontSize: 9, color: "#00F5FF", fontFamily: "monospace", textTransform: "uppercase", letterSpacing: "0.15em" }}>LIVE APPLICATION SUITE & CODE INSPECTOR</div>
                  <div style={{ fontSize: 17, fontWeight: 900, color: "#fff", fontFamily: "'Space Grotesk', sans-serif", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    <span>🚀 {codeModalSlug}</span>
                    <span style={{ color: "#00FF88", fontSize: 11, fontFamily: "monospace", background: "rgba(0,255,136,0.12)", border: "1px solid rgba(0,255,136,0.3)", padding: "2px 8px", borderRadius: 6 }}>● 100% Live & Interactive</span>
                  </div>
                </div>

                {/* Primary Mode Switcher */}
                <div style={{ display: "flex", background: "rgba(2,6,23,0.8)", border: "1px solid rgba(0,245,255,0.25)", borderRadius: 10, padding: 3, gap: 4 }}>
                  <button
                    onClick={() => setCodeModalViewMode("preview")}
                    style={{
                      padding: "6px 14px", borderRadius: 7, fontSize: 11, fontWeight: 800, fontFamily: "monospace", cursor: "pointer",
                      background: codeModalViewMode === "preview" ? "linear-gradient(90deg, #00F5FF, #6E56FF)" : "transparent",
                      border: "none", color: codeModalViewMode === "preview" ? "#fff" : "rgba(148,163,184,0.8)",
                      boxShadow: codeModalViewMode === "preview" ? "0 0 12px rgba(0,245,255,0.4)" : "none"
                    }}
                  >
                    🌐 Live Interactive Preview
                  </button>
                  <button
                    onClick={() => setCodeModalViewMode("code")}
                    style={{
                      padding: "6px 14px", borderRadius: 7, fontSize: 11, fontWeight: 800, fontFamily: "monospace", cursor: "pointer",
                      background: codeModalViewMode === "code" ? "linear-gradient(90deg, #6E56FF, #00FF88)" : "transparent",
                      border: "none", color: codeModalViewMode === "code" ? "#fff" : "rgba(148,163,184,0.8)",
                      boxShadow: codeModalViewMode === "code" ? "0 0 12px rgba(0,255,136,0.4)" : "none"
                    }}
                  >
                    💻 3-Tier Codebase
                  </button>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <a
                    ...
                    style={{ textDecoration: "none", padding: "6px 14px", borderRadius: 8, background: "linear-gradient(90deg,#00F5FF,#00FF88)", color: "#020617", fontSize: 11, fontWeight: 900, fontFamily: "monospace", display: "flex", alignItems: "center", gap: 6, boxShadow: "0 0 16px rgba(0,245,255,0.3)" }}
                  >
                    🚀 Open Live Site ↗
                  </a>
                  <button
                    ...
                    style={{ padding: "6px 12px", borderRadius: 8, background: "rgba(0,245,255,0.12)", border: "1px solid #00F5FF", color: "#00F5FF", fontSize: 11, fontWeight: 700, fontFamily: "monospace", cursor: "pointer" }}
                  >
                    📋 Copy Code
                  </button>
                  <button
                    ...
                    style={{ padding: "6px 12px", borderRadius: 8, background: "rgba(0,255,136,0.12)", border: "1px solid #00FF88", color: "#00FF88", fontSize: 11, fontWeight: 700, fontFamily: "monospace", cursor: "pointer" }}
                  >
                    📥 Download
                  </button>
                  <button onClick={() => setCodeModalOpen(false)} style={{ background: "none", border: "none", color: "rgba(148,163,184,0.6)", cursor: "pointer", padding: 4 }}>
                    <X style={{ width: 20, height: 20 }} />
                  </button>
                </div>
              </div>

              {/* VIEW MODE: 1. LIVE INTERACTIVE APP PREVIEW */}
              {codeModalViewMode === "preview" && (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10, overflow: "hidden" }}>
                  {/* Viewport & Device Controls */}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(15,23,42,0.6)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "6px 12px", flexWrap: "wrap", gap: 8 }}>
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      <span style={{ fontSize: 10, color: "var(--text-muted)", fontFamily: "monospace" }}>DEVICE:</span>
                      <button
                        onClick={() => setPreviewDevice("desktop")}
                        style={{ padding: "3px 10px", borderRadius: 6, fontSize: 10, fontFamily: "monospace", fontWeight: 700, cursor: "pointer", background: previewDevice === "desktop" ? "rgba(0,245,255,0.2)" : "transparent", border: `1px solid ${previewDevice === "desktop" ? "#00F5FF" : "transparent"}`, color: previewDevice === "desktop" ? "#00F5FF" : "#94a3b8" }}
                      >
                        🖥️ Desktop (100%)
                      </button>
                      <button
                        onClick={() => setPreviewDevice("tablet")}
                        style={{ padding: "3px 10px", borderRadius: 6, fontSize: 10, fontFamily: "monospace", fontWeight: 700, cursor: "pointer", background: previewDevice === "tablet" ? "rgba(0,245,255,0.2)" : "transparent", border: `1px solid ${previewDevice === "tablet" ? "#00F5FF" : "transparent"}`, color: previewDevice === "tablet" ? "#00F5FF" : "#94a3b8" }}
                      >
                        📱 Tablet (768px)
                      </button>
                      <button
                        onClick={() => setPreviewDevice("mobile")}
                        style={{ padding: "3px 10px", borderRadius: 6, fontSize: 10, fontFamily: "monospace", fontWeight: 700, cursor: "pointer", background: previewDevice === "mobile" ? "rgba(0,245,255,0.2)" : "transparent", border: `1px solid ${previewDevice === "mobile" ? "#00F5FF" : "transparent"}`, color: previewDevice === "mobile" ? "#00F5FF" : "#94a3b8" }}
                      >
                        📲 Mobile (390px)
                      </button>
                    </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ fontSize: 10, fontFamily: "monospace", color: "#38bdf8", background: "rgba(56,189,248,0.1)", padding: "3px 8px", borderRadius: 6, border: "1px solid rgba(56,189,248,0.2)" }}>
                        URL: {liveAppUrl}
                      </div>
                      <button
                        onClick={() => setPreviewKey(k => k + 1)}
                        style={{ padding: "3px 10px", borderRadius: 6, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.15)", color: "#cbd5e1", fontSize: 10, fontFamily: "monospace", cursor: "pointer" }}
                      >
                        🔄 Reload Frame
                      </button>
                    </div>
                  ...
                  <div style={{ flex: 1, background: "#020617", border: "1px solid rgba(0,245,255,0.25)", borderRadius: 14, overflow: "hidden", display: "flex", justifyContent: "center", alignItems: "stretch", position: "relative" }}>
                    <iframe
                      key={previewKey}
                      src={liveAppUrl}
                      title={`Live App Preview - ${codeModalSlug}`}
                      style={{
              ...
              {/* VIEW MODE: 2. 3-TIER SOURCE CODEBASE EXPLORER */}
              {codeModalViewMode === "code" && (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10, overflow: "hidden" }}>
                  {/* 3-Tier Tab Bar */}
