...
export default function SideRobot() {
  ...
  const [robotPos, setRobotPos] = useState(() => localStorage.getItem("nexus_siderobot_position") || "bottom-right");

  useEffect(() => {
    const handlePosChange = () => {
      setRobotPos(localStorage.getItem("nexus_siderobot_position") || "bottom-right");
    };
    window.addEventListener("siderobot_position_changed", handlePosChange);
    return () => window.removeEventListener("siderobot_position_changed", handlePosChange);
  }, []);
  ...
  const getPositionStyles = () => {
    switch (robotPos) {
      ...
      case "top-right":
        return { top: 70, right: 16, alignItems: "flex-end" };
      case "top-left":
        return { top: 70, left: 16, alignItems: "flex-start" };
      case "bottom-right":
      default:
