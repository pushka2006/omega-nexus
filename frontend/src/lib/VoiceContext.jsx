...
export function VoiceProvider({ children }) {
  ...
  const executeVoiceCommand = useCallback(async (transcript) => {
    ...
    // Safe Window Open helper with popup-blocker fallback to direct navigation
    ...
    // 2.5 EXTERNAL WEB NAVIGATION VOICE COMMANDS (YouTube, GitHub, Google, Facebook, Amazon)
    if (
      clean.includes("github") ||
      clean.includes("git hub") ||
      clean.includes("git-hub") ||
      clean.includes("get hub") ||
      clean.includes("gidhub") ||
      clean.includes("git hop") ||
      clean.includes("git top") ||
      clean.includes("gitlab")
    ) {
      speak("Opening GitHub.");
      toast.success("Opening GitHub...");
      safeOpenUrl("https://github.com");
      return;
    }
    if (
      clean.includes("youtube") ||
      clean.includes("you tube") ||
      clean.includes("utube") ||
      clean.includes("u tube")
    ) {
    ...
    if (
      ...
      clean.includes("goggle") ||
      clean.includes("goo gle")
    ) {
    ...
    if (
      ...
      clean.includes("face book") ||
      clean.includes("fb.com")
    ) {
      speak("Opening Facebook.");
      toast.success("Opening Facebook...");
      safeOpenUrl("https://www.facebook.com");
      return;
    }
    if (
      clean.includes("amazon") ||
      clean.includes("ama zon") ||
      clean.includes("amzon")
    ) {
      speak("Opening Amazon.");
      toast.success("Opening Amazon...");
      safeOpenUrl("https://www.amazon.com");
      return;
    }
    ...
    if (clean === "help" || clean.includes("what can i say") || clean.includes("list commands")) {
      speak("Valid voice directives include: open youtube, open github, open google, open facebook, open amazon, navigate sector, run command, or lock system.");
      toast.info("Try: 'open youtube', 'open github', 'open google', 'open facebook', 'open amazon'");
      return;
    }
